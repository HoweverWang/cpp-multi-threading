...
public:
    ...
    ThreadPool(size_t t_num): m_stop(false)
    { init(t_num); }
    ...
    auto submitTask(Func&& func, Args&&... args) {
        ...
        using returnType = std::invoke_result_t<Func, Args...>; // 获得返回值类型，更先进的写法
        ...
        std::future<returnType> res = task->get_future();
        {
            ...
            Q.emplace([task]() { (*task)(); }); // 之前task用了共享指针，从而避免脱离作用域被析构后 Q里的task失效
        }
        m_cv.notify_one();
        return res;
    }
